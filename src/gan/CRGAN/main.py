...
from models.sngan_32 import ICRGenerator, ICRDiscriminator


def get_models(cfg):

    netG = ICRGenerator(nz=cfg.model.GAN.nz,
                        ngf=cfg.model.GAN.ngf,
                        loss_type=cfg.model.GAN.loss_type,
                        is_amp=cfg.training.is_amp)

    netD = ICRDiscriminator(ndf=cfg.model.GAN.ndf,
                            loss_type=cfg.model.GAN.loss_type,
                            is_amp=cfg.training.is_amp,
                            is_transforms=cfg.model.ICR.is_transforms,
                            real_lambda=cfg.model.ICR.real_lambda,
                            fake_lambda=cfg.model.ICR.fake_lambda)
