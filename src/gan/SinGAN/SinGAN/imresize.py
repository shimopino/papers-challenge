...
def np2torch(x, opt):
    """
    This is equivalent to Combination of (ToTensor & Normalize)

    Arguments:
        x {ndarray} -- ndarray input image
        opt {dict} -- check #of image chennels and cuda availability

    Returns:
        [torch.Tensor] -- float32 Tensor [BCHW]
    """
    if opt.nc_im == 3:
        # [H,W,C] => [H,W,C,B] => [B,C,H,W]
        x = x[:, :, :, None]
        x = x.transpose((3, 2, 0, 1)) / 255
    else:
        # [H,W,C] => [H,W] => [H,W,C,B] => [B,C,H,W]
        x = color.rgb2gray(x)
        x = x[:, :, None, None]
    ...
    if opt.is_cuda:
        x = move_to_gpu(x)
    x = x.type(torch.cuda.FloatTensor) if (opt.is_cuda) else x.type(torch.FloatTensor)
    x = norm(x)
    return x
...
def torch2uint8(x):
    """
    convert Float32 torch.Tensor to Uint8 ndarray

    Arguments:
        x {torch.Tensor} -- #of samples is 1 [1CHW]

    Returns:
        [ndarray] -- uint8 ndarray [HWC]
    """
    x = x[0, :, :, :]
    x = x.permute((1, 2, 0))
    x = 255 * denorm(x)
