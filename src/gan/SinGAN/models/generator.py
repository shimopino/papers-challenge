...
class Generator(nn.Module):
    def __init__(self, img_size_min, num_scale, scale_factor=4 / 3):
        ...
        self.size_list = [
            int(self.img_size_min * scale_factor ** i) for i in range(num_scale + 1)
        ]
        ...
        first_generator.append(
            nn.Sequential(
                nn.Conv2d(3, self.nf, 3, 1), nn.BatchNorm2d(self.nf), nn.LeakyReLU(2e-1)
            )
        )
        for _ in range(3):
            first_generator.append(
                nn.Sequential(
                    nn.Conv2d(self.nf, self.nf, 3, 1),
    ...
    def forward(self, z, img=None):
        ...
        for i in range(1, self.current_scale + 1):
            x_inter = F.interpolate(
                x_inter,
                (self.size_list[i], self.size_list[i]),
                mode="bilinear",
                align_corners=True,
            )
            x_prev = x_inter
...
if __name__ == "__main__":
    ...
    model = Generator(img_size_min, num_scale, scale_factor)
    model.progress()
